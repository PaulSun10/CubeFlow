...
- **Phase 2A.1: complete; passed physical validation.** GAN Gen3/Gen4 history recovery scheduling and H-perm/M-slice recognition latency. Preserve normal timing/coalescing; no speculative protocol changes.
- **Phase 2B: complete; physically validated checkpoint.** Target-preserving Recovery-versus-Replan decision above the canonical feed, using a proven current-state anchor.
...
### Phase 2B Decision And Validation

Recovery remains the correctness foundation and immediate guidance for one- or two-move corrections. From three correction moves onward, Replan is the primary presentation path: retain the best executable Recovery internally as a safety fallback, but do not transiently publish its long move sequence while the verified target-preserving candidate is computed. First use the existing <=3 HTM optional shortcut, then compare the best correction HTM plus uncompleted current-plan tokens against the candidate; partial half turns retain one remaining action. A verified Replan wins whenever it is strictly shorter than the executable Recovery path; equal, longer, unavailable, invalid or otherwise rejected candidates restore the request's identity-bound Recovery fallback. The former six-move and 25% savings gates remain removed.

The original generated scramble permanently defines the authoritative target and remains the saved/exported scramble. Replan computes `inverse(current trustworthy state) * original target` and uses the existing bounded min2phaseCXX dependency to generate a direct current-to-target sequence of at most 21 HTM. Swift independently applies every returned move to the captured current facelets and requires the final state to equal the unchanged original target before adoption. Invalid states, unavailable/out-of-bound solutions and failed target verification fall back to Recovery. This is execution guidance toward the existing random-state target, not generation of a different target.

An adopted replan becomes the sole current execution plan; existing state-based commuting/partial/undo verification, Collapse/Trail presentation and guaranteed Recovery continue against it. At the correction-to-remaining-plan boundary, adjacent turns of the same face are normalized modulo four before cost comparison, verification, highlighting and Ready evaluation. Any original tokens absorbed by that normalization are marked as superseded in the executable plan, so presentation, workload and verifier progression all use the same route; for example, original `F` after an actual `F'` becomes only `F2`, never `F F2`. Cancellation can advance directly to the resulting verified checkpoint without changing the immutable target. A later deviation may replan again to the same original target. A candidate beginning with the inverse of the triggering move carries a one-move switching penalty, rejecting only a marginal immediate undo while allowing a clearly shorter path. Obsolete plans cannot return because adoption replaces the current progress model and increments its generation.

Planning is serialized off MainActor and stale queued work is cancellable. Adoption checks scramble epoch/version, exact source facelets, unchanged target, current replan generation, trusted continuity, deviation and idle preparation lifecycle. A new epoch is established on adoption without altering first/last move timing or GAN history/coalescing. If a genuine continuity break removes the reversible trail, the next authoritative snapshot can plan directly from that trustworthy state to the same immutable target; an equal-valued snapshot also restores canonical trust. Existing presentation snapshots retain the current plan and generation; no second session owner was introduced.

Timer-initiated connection readiness is separate from the low-level BLE `connected` state. The current connection attempt must still match, its reset-policy decision must be resolved, authoritative or locally confirmed facelets must exist, and canonical continuity/state must be trusted before move observation is presented as ready. Until then the Timer replaces Virtual Cube/guidance with Loading; readiness restores them and emits the shared Connected success HUD once for that attempt. Peripheral callbacks and delayed initialization requests are device/attempt guarded, and reconnect creates a new readiness transition. No pre-readiness turns are inferred.

Reset capability is protocol-owned rather than Timer-owned. GAN Gen2/Gen3/Gen4 use their established device-side reset command, remain untrusted until an authoritative facelet read-back arrives, and accept the actual device snapshot rather than assuming write success means solved. MoYu/MY32 has no established safe internal-state reset command, so reset uses a per-device software reference anchored to an authoritative raw snapshot. Reconnect without reset preserves the locally confirmed state only when the unchanged device snapshot matches that reference; a different snapshot discards the reference and restores device truth without inventing disconnected move history. Reset, failed history and explicit resync remain continuity boundaries.

Each Session is the authoritative owner of its selected event and Timing Method. Timer and Settings restore both through the same Core Data-backed configuration path when the selected Session changes or the app relaunches, and changes are written back only to that active Session. Existing Sessions without the optional Timing Method value migrate once from the legacy global preference, after which the persisted Session value wins; new Sessions default to 3x3 and Timer. Backups preserve both fields while remaining compatible with older backup payloads.
